...
namespace PDCUtility
{
    ...
    {
        public static string ConvertDecimalToFraction(this decimal pvalue,
                                        bool skip_rounding = false,
                                        decimal dplaces = (decimal)0.0625)
        {
            ...
            if (value == System.Math.Round(value, 0)) // whole number check
                return value.ToString();

            // get the whole value of the fraction
            decimal mWhole = System.Math.Truncate(value);
            ...
            if (mFraction > 0m)
            {
                ...
                mNumerator = (uint)System.Math.Pow(10, intFractLength);
        ...
        public static decimal ConvertFractionToDecimal(string value)
        {
            string[] dparse;
            string[] fparse;
            string whole = "0";
        ...
        public static bool ReformatForFeetAndInches(ref string str, bool zero_for_blank = true)
        {
            ...
            decimal d1 = System.Math.Floor(d);
            decimal d2 = d - d1;
            d2 = System.Math.Round(d2 * 12, 2);

            string s1;
            string s2;

            s1 = d1 == 0 ? "" : d1.ToString() + "'";
            s2 = d2 == 0 ? "" : FractionConverter.ConvertDecimalToFraction(d2) + @"""";

            str = string.Format(@"{0} {1}", s1, s2).Trim();
        ...
        {
            string sPlaces = FractionConverter.ConvertDecimalToFraction(places, true);
            string[] s = sPlaces.Split('/');

            if (s.Count() == 2)
            {
                int nPlaces = System.Convert.ToInt32(s[1]);
                decimal d = System.Math.Round(val * nPlaces);
                return d / nPlaces;
            }
